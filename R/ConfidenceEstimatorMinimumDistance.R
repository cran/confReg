########################################## CONFIDENCE ESTIMATOR MINIMUM DISTANCE ################################

setMethod("ConfidenceEstimatorMinimumDistance", signature("Regression", "Dataset" ), function(regressionModel, trainingData, qualityFunction = averageError) {
	new("ConfidenceEstimatorMinimumDistance", regressionModel, trainingData, qualityFunction);
})


setMethod("create", signature("ConfidenceEstimatorMinimumDistance","logical", "vector"), function(.Object, optimize, predictionsOfTrainingData) {
	callNextMethod();
...
setMethod("estimate", signature("ConfidenceEstimatorMinimumDistance", "Dataset", "vector"), function(.Object, testData, predictionsOfTestData) {
	callNextMethod();
		...
		if (min(errors^2) < errorThreshold)
		{
			confidences <- apply(distMatrix,1,function(x) {
				...
				if (length(which(errors^2<=errorThreshold))<1)
					...
					return( 1 - min(x[which(errors^2<=errorThreshold)]) )
				});	
		} 
	} else {
		confidences <- apply(distMatrix,1,function(x) {
