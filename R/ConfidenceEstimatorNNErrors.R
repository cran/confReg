########################################## CONFIDENCE ESTIMATOR NN ERRORS ################################

setMethod("ConfidenceEstimatorNNErrors", signature("Regression", "Dataset" ), function(regressionModel, trainingData, qualityFunction = averageError) {
	new("ConfidenceEstimatorNNErrors", regressionModel, trainingData, qualityFunction);
})


setMethod("create", signature("ConfidenceEstimatorNNErrors","logical", "vector"), function(.Object, optimize, predictionsOfTrainingData) {
	callNextMethod();
...
setMethod("estimate", signature("ConfidenceEstimatorNNErrors", "Dataset", "vector"), function(.Object, testData, predictionsOfTestData) {
	callNextMethod();
	...
	if (nns != -1)
	{
		...
		confidences <- apply(neighbors,2,function(x) if(length(x)<2) return(-500) else return(1-mean( errors[x]^2) ));

	} else {
		# predict confidence scores by kernel density estimate
		confidences <- apply(distMatrix,1,function(x) 1 - sum(exp(-0.5*x^2) * (errors^2)) / sum(exp(-0.5*x^2))  );
	}
